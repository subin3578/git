                        ...
                        <section class="list">
                                ...
                                <table border= "0">
				    				...
				    				<c:forEach var="article" items="${articles}">
				                        <tr>
				                        	<!-- 여기 순서번호 수정 해야함 -->
				                            <td>${pageStartNum}</td>
				                            <td><a href="/farm/croptalk/storyview.do?no=${article.no}">${article.title}</a>&nbsp;[${article.comment}]</td>
				                            <td>${article.nick}</td>
				                            <td>${article.rdate}</td>
				                            <td>${article.hit}</td>
				                        </tr>
                            ...
                            <div class="paging">
							    <c:if test="${pageGroup.start > 1}">
							        <a href="/farm/croptalk/story.do?pg=${pageGroup.start-1}" class="prev">이전</a>
							    </c:if>
							    ...
							    <c:forEach var="i" begin="${pageGroup.start}" end="${endPage}">
							        <a href="/farm/croptalk/story.do?pg=${i}" class="num ${currentPage == i ? 'current':'off'}">${i}</a>
							    </c:forEach>
							    <c:if test="${endPage < lastPageNum}">
							        <a href="/farm/croptalk/story.do?pg=${endPage+1}" class="next">다음</a>
							    </c:if>
							</div>
                            <!-- 글쓰기 버튼 -->
 							 <a href="/farm/croptalk/storywrite.do" class="btnWrite">글쓰기</a>
